  ...
  <div class="container-sm text-center position-absolute top-50 start-50 translate-middle">
    <div class="card">
      <div class="card-body">
        ...
        $sql = "UPDATE `pessoas` set `name` = '$name', `address` = '$address', `phone` = '$phone', `email` = '$email', `birthDate` = '$birthDate' WHERE id = $id";

        if (mysqli_query($conn, $sql)) {
          mensagem("$name Alterado com sucesso!", 'primary');
        } else {
          mensagem("$name NÃO alterado!", 'danger');
        }
        ?>
